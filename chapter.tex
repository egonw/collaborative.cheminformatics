...
part of cheminformatics as most cheminformatics methodologies are
developed using real data, which must also be tested on real data for
it to be used in experimental settings. \ola{Why is this the
  case?}. However, as the field matured it became more specialized,
...
Subversion (SVN), Mercurial, Bazaar, and Git \ola{Should we have
  references to these?} \rg{I think links will be sufficient}. CVS is the oldest and mostly replaced
...
Another aspect that simplifies collaboration is to use an
...
licenses, and the MIT and BSD licenses \ola{Should we write out what
  the abbreviations stand for, e.g. Gnu General Public License?}
\rg{I'd probably skip that, as the references should be enough}. A full overview of Open
Source licenses is available from the Open Source Initiative:
\url{http://www.opensource.org/licenses}.

\subsection{Communication}

\rg{Maybe this section could be shortened and the bulk of it moved to
  the Managing ... section, later on}
...
need to be discussed, and solutions proposed. Mailing lists have 
...
in scientific publishing is well recognized, it is seldom
...
and replies are provided in various formats, including Resource Description Framework [REF],
the underlying technology of the Semantic Web [REF].
...
services\cite{Dong:2007Web} that provide access to core cheminformatics methods
